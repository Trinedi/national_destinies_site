...
def collect_needed_keys(web_data_dir: Path) -> set[str]:
    """Read the other parsed JSONs to find which loc keys the frontend needs."""
    needed: set[str] = set()
    ...
    if formables_path.exists():
        ...
        for rec in data.get("formables", []):
            ...
            if bk:
                needed.add(bk)
                needed.add(bk + "_desc")
                # Some formable definitions key descriptions on tag-only.
                if bk.endswith("_f"):
                    needed.add(bk[:-2])
...
def main() -> int:
    ...
    if not args.full:
        needed = collect_needed_keys(web_data_dir)
        before = len(loc)
        loc = {k: v for k, v in loc.items() if k in needed}
        print(f"  filtered to {len(loc)}/{before} keys "
              f"({len(needed)} requested by formables/geography)")
